...
namespace OFXSharp
{
    ...
    #region CreditCardBalance
    public class CreditCardBalance : AccountBalance
    {
        public decimal LedgerBalance { get; set; }
        public DateTime LedgerBalanceDate { get; set; }
        public DateTime AvailableBalanceDate { get; set; }

        public CreditCardBalance(XmlNode ledgerNode, XmlNode availableNode)
        {
            var tempLedgerBalance = ledgerNode.GetValue("//BALAMT");
            ...
            // ***** OFX files from my bank CC don't have the 'avaliableNode' node, so i manage a null situation
            if (availableNode == null)
            {
                AvailableBalance = 0;
